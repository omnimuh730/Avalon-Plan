...
import { accountInfoCollection, llmCallLogCollection } from "../db/mongo.js";
 ...
 * All registered users with configured API keys (masked) and LLM spend from llm_call_log.
 */
export const getAiUsageMonitor = createAsyncHandler(async (req, res) => {
  if (!accountInfoCollection || !llmCallLogCollection) {
    return res.status(503).json({ error: "Database not ready" });
  }
