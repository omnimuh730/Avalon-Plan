...
import { listPostedJobsFromAthens } from "../athens-client.mjs";
...
export async function listPostedJobs({ source, applierId, skip = 0, limit = 50, includeContent = false }) {
  if (USE_ATHENS_REST && CONFIG.athensServerUrl) {
    try {
      const jobs = await listPostedJobsFromAthens({ applierId, source, skip, limit });
      return jobs.map((j) => ({
        id: j.id,
        title: j.title,
        company: j.company,
      ...
      console.warn("[connector] Athens REST listPostedJobs failed, falling back to Mongo:", err.message);
    }
  }
  const db = await getDb();
  const applierOid = toOid(applierId);
