...
import { listProfiles, getProfileById, getProfileResumes } from "../core/resumes.mjs";
import { listOpenAiModels, DEEPSEEK_MODELS, isDeepSeekModel } from "../core/models.mjs";
import { emptyUsage, mergeUsage, formatUsd } from "../core/pricing.mjs";
import { listPostedJobs, listAppliedJobs, listJobsByIds, postedSourceCounts, markJobApplied, dashboardStats } from "../core/jobs.mjs";
import { countByStatus, listActivityEntries, listFailedAttempts } from "../core/applications-log.mjs";
import {
  generateRunId,
  createRun,
  appendRunEvent,
  ...
  listDashboardJobs,
  dashboardRunStats,
} from "../core/agent-runs.mjs";
import { JobSource } from "../core/job-sources.mjs";
import { getDb } from "../core/db.mjs";
import { listScheduledJobs } from "../core/jobs.mjs";
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === "/api/health") {
    ...
    return sendJSON(res, 200, {
      ...
      playwrightCwd: PATHS.agentRuntime,
      applicationsLog: `${PATHS.agentRuntime}/logs/applications.jsonl`,
    });
  }
  ...
  if (runScreenshotMatch && req.method === "GET") {
    try {
      ...
      const runsDir = path.resolve(`${PATHS.agentRuntime}/logs/runs`);
  ...
  if (pathname === "/api/deploy" && req.method === "POST") {
    ...
    const mode = (body.mode || CONFIG.defaultMode || "plan").trim();
...
markInterruptedRuns()
  .then(() => {
    initConnectorSocket(server);
    server.listen(CONFIG.port, () => {
      console.log(`\n  connector  →  http://localhost:${CONFIG.port}`);
