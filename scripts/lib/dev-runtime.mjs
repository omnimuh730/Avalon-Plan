...
export const backendServices = [
	{ name: 'unified-ai', label: 'Unified AI', cmd: 'npm', args: ['run', 'start', '-w', 'unified-ai-server'], cwd: ROOT },
	{ name: 'athens-server', label: 'Athens-server', cmd: 'npm', args: ['run', 'start', '-w', 'Athens-server'], cwd: ROOT },
	{ name: 'avalon-backend', label: 'Avalon relay', cmd: 'npm', args: ['run', 'dev:backend'], cwd: AVALON },
	{ name: 'avalon-ai-bff', label: 'Avalon AI BFF', cmd: 'npm', args: ['run', 'dev:ai-bff'], cwd: AVALON },
];
...
export const backendPorts = [
	{ host: '127.0.0.1', port: Number(process.env.UNIFIED_AI_PORT || 8790), label: 'Unified AI', service: 'unified-ai' },
	{ host: '127.0.0.1', port: Number(process.env.ATHENS_SERVER_PORT || 8979), label: 'Athens-server', service: 'athens-server' },
	{ host: '127.0.0.1', port: Number(process.env.AVALON_PORT || 3847), label: 'Avalon relay', service: 'avalon-backend' },
	{ host: '127.0.0.1', port: Number(process.env.AI_BFF_PORT || 3920), label: 'Avalon AI BFF', service: 'avalon-ai-bff' },
];
...
export function getDevSummary() {
	...
	return {
		...
		endpoints: [
			...
			{ label: 'Unified AI', url: 'http://localhost:8790' },
			{ label: 'Avalon relay', url: 'http://localhost:3847' },
			{ label: 'Avalon AI BFF', url: 'http://localhost:3920' },
		],
	};
}
