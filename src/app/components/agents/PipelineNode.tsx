...
export function PipelineNodeCard({ node }: { node: PipelineNode }) {
  ...
  return (
    <div
      className={cn(
        "absolute w-[180px] bg-card border rounded-xl p-4 shadow-md transition-all",
        s.border,
    ...
    >
      ...
      {node.metrics && (
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          {node.metrics.count !== undefined && (
            <span className="font-semibold">{node.metrics.count}</span>
          )}
