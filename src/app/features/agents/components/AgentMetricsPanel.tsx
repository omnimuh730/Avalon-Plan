...
const DEPENDENCY_FIELDS = ["match_score", "role_title", "company_name", "priority_level"];
  ...
  return (
    <aside className="w-80 border-l border-border bg-card flex flex-col overflow-y-auto subtle-scroll flex-shrink-0">
      ...
      <div className="p-5 space-y-5 flex-1">
        ...
        <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-xl p-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{agent.model} · Latency {agent.latencyMs}ms</p>
          </div>
        ...
        <Collapsible title="Normalized Output" defaultOpen>
          <pre className="bg-secondary/70 border border-border rounded-lg p-3 text-xs text-foreground/80 overflow-x-auto leading-relaxed" style={mono}>
        ...
        <Collapsible title="Dependency Tree">
          <div className="space-y-2 text-sm">
            {DEPENDENCY_FIELDS.map((field) => (
              <div key={field} className="flex items-center gap-2 px-3 py-2 bg-secondary/50 rounded-lg">
      ...
      <div className="p-5 border-t border-border">
        <button type="button" className="w-full bg-primary text-white rounded-xl py-3 text-sm font-bold hover:bg-primary/90 transition-colors shadow-sm min-h-10">
          Apply Normalization
        </button>
      </div>
    </aside>
