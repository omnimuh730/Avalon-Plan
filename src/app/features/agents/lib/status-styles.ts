...
export function jobStatusStyle(s: JobStatus): { dot: string; badge: string; text: string; label: string } {
  switch (s) {
    ...
    default:
      return { dot: "bg-gray-400", badge: "bg-gray-50 border-gray-200", text: "text-gray-600", label: String(s) };
  }
}
...
export function runStatusStyle(s: string): { dot: string; label: string; labelClass: string } {
  switch (s) {
    ...
    case "paused":
      return { dot: "bg-amber-500", label: "Paused", labelClass: "text-amber-700 bg-amber-50 border-amber-200" };
    case "interrupted":
