...
import { COPILOT_CONVERSATIONS } from "../../../data/copilot";
...
export function ConversationSidebar({ activeConv, onSelect }: ConversationSidebarProps) {
  return (
    <div className="w-52 border-r border-border flex flex-col flex-shrink-0 bg-secondary/30">
      <div className="p-4 border-b border-border">
        <button type="button" className="w-full flex items-center justify-center gap-2 bg-primary text-white px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-primary/90 transition-colors min-h-10">
          <Plus className="w-4 h-4" />
          New chat
        </button>
      </div>
      <div className="flex-1 p-3 space-y-1 overflow-y-auto subtle-scroll">
        {COPILOT_CONVERSATIONS.map((c, i) => (
          <button
            key={c}
            type="button"
            onClick={() => onSelect(i)}
