            ...
            <div className="space-y-4">
              {[
                ...
                <div key={f.label}>
                  ...
                  <input defaultValue={f.value} className="w-full bg-secondary border border-border rounded-xl px-4 py-3 text-sm text-foreground outline-none focus:border-primary/40 min-h-10" />
                </div>
              ))}
            </div>
            <button type="button" className="mt-5 bg-primary text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-primary/90 min-h-10">Save Changes</button>
          </div>
        </div>
      )}
      {tab === "integrations" && (
        <div className="space-y-4 max-w-2xl">
          {INTEGRATIONS.map((int) => (
            <div key={int.n} className="bg-card border border-border rounded-xl p-5 flex items-center gap-5 hover:shadow-md transition-all shadow-sm">
              <div className="w-12 h-12 rounded-xl bg-secondary flex items-center justify-center flex-shrink-0"><Globe className="w-6 h-6 text-muted-foreground" /></div>
              <div className="flex-1 min-w-0"><p className="text-base font-bold text-foreground">{int.n}</p><p className="text-sm text-muted-foreground">{int.d}</p></div>
              <div className="flex items-center gap-4 flex-shrink-0">
                ...
                <button type="button" className={cn("text-sm px-4 py-2.5 rounded-xl font-bold transition-colors min-h-10", int.st === "connected" ? "bg-secondary text-muted-foreground hover:text-foreground border border-border" : "bg-primary text-white hover:bg-primary/90")}>{int.st === "connected" ? "Manage" : "Connect"}</button>
              </div>
            </div>
          ))}
        </div>
