// Read the agent-runtime applications audit log (JSONL) for activity feeds and status stats.
...
const DEFAULT_LOG = path.resolve(__dirname, "..", "..", "agent-runtime", "logs", "applications.jsonl");
  ...
  for (const line of readLogLines(logPath, 500).reverse()) {
    try {
      ...
      rows.push({
        ...
        agentName: row.source === "agentforce" ? (row.profile || "Agent") : "agent-runtime",
        profile: row.profile || "",
        company: row.company || "",
        role: row.role || "",
        status,
