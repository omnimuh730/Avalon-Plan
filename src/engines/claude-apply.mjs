...
export async function runApplicationClaude({
  ...
  // MCP mode: launch REAL Chrome on a per-run copy of the applicant's PERSISTENT
  // profile (concurrent via per-run dir; logins from prior runs carried in). Run
  // from a CLAUDE.md-free temp dir so the agent uses the MCP (not the CLI).
  ...
  // Persist the run's Chrome profile back to the applicant's master, so any login
  // / verification done this run is reused next time (no re-verify).
  if (mcpBuilt) persistProfileBack(mcpBuilt);
